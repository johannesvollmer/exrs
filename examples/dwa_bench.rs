...
//!
//! To also print the per-phase timing breakdown (setup/entropy/dequant/idct/...):
//!   RUSTFLAGS="-C target-cpu=native" cargo build --release --example dwa_bench --features dwa-profile
//!
//! Usage:
...
fn main() {
    ...
    for it in 0..iters {
        ...
        exr::compression::dwa_profile::reset_all();
        ...
        #[cfg(feature = "dwa-profile")]
        if it == iters - 1 {
            exr::compression::dwa_profile::report();
        }
    ...
    println!("exrs (Rust) parallel={} best={:.2} ms avg={:.2} ms", parallel, best, total / iters as f64);
}
