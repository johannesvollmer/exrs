...
impl CompressedDeepTileBlock {
    ...
    pub fn read(read: &mut impl Read, hard_max_block_byte_size: usize) -> Result<Self> {
        let coordinates = TileCoordinates::read(read)?;
        let compressed_pixel_offset_table_size = u64_to_usize(u64::read(read)?,"deep table size")?;
        let compressed_sample_data_size = u64_to_usize(u64::read(read)?, "deep size")?; // TODO u64 just guessed
        let decompressed_sample_data_size = u64_to_usize(u64::read(read)?, "raw deep size")?;
...
impl Chunk {
    ...
    pub fn write(&self, write: &mut impl Write, header_count: usize) -> UnitResult {
        ...
        if header_count != 1 {  usize_to_i32(self.layer_index, "layer index")?.write(write)?; }
