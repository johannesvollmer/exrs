        ...
        return (start_y .. start_y + height)
            .flat_map(move |absolute_y| {
                channels.iter().enumerate()
                    .map(move |(chan_index, chan)| (absolute_y, chan, chan_index))
            })
            .filter(|(absolute_y, channel, _)| subsampled_image_contains_line(
                usize_to_i32(channel.sampling.y()),
                usize_to_i32(*absolute_y)
            ))
            .scan(0_usize, move |byte, (absolute_y, channel, chan_index)| {
                ...
                *byte += byte_len;
                ...
                Some((byte_range, LineIndex {
                    ...
                    sub_sample_count: channel.subsampled_line_pixels(width)
                }))
            });
    }
}
