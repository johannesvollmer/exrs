...
pub mod io {
    ...
    pub use super::{ReadResult, ReadError, WriteResult, WriteError};
    ...
    pub fn read_f16_vec<R: ReadBytesExt>(read: &mut R, data_size: usize, estimated_max: usize) -> ReadResult<Vec<f16>> {
        let u16_vec = if data_size < estimated_max {
            let mut data = vec![0; data_size];
        ...
        // reinterpret the u16 array as newtype f16(u16)
        // FIXME i have no idea if that works
        /*let f16_vec = unsafe { Vec::from_raw_parts(
            u16_vec.as_ptr(), u16_vec.len(), u16_vec.capacity()
        ) };

        ::std::mem::forget(u16_vec);
        Ok(f16_vec)*/

        // TODO just reinterpret the u16 array as f16?
        Ok(u16_vec.into_iter().map(|u| f16::from_bits(u)).collect())
    }
    ...
    #[must_use]
    pub fn read_file(path: &::std::path::Path) -> ReadResult<File> {
        read(::std::fs::File::open(path)?)
    }

    /// assumes that the provided reader is not buffered, and will create a buffer for it
    #[must_use]
    pub fn read<R: Read + Seek>(unbuffered: R) -> ReadResult<File> {
        read_seekable_buffer(&mut SeekBufRead::new(unbuffered))
    }

    #[must_use]
    pub fn read_seekable_buffer<R: Read + Seek>(read: &mut SeekBufRead<R>) -> ReadResult<File> {
        MagicNumber::validate_exr(read)?;
        let meta_data = meta::MetaData::read(read)?;
        let chunks = chunks::Chunks::read(read, &meta_data)?;
        Ok(::file::File { meta_data, chunks, })
    }


    #[must_use]
    pub fn write_file(path: &str, image: &File) -> WriteResult {
        write(&mut ::std::fs::File::open(path)?, image)
    }

    #[must_use]
    pub fn write<W: Write>(write: &mut W, image: &File) -> WriteResult {
