...
use super::{mod_p, usize_to_i32, ByteVec, ChannelList, Error, IntegerBounds, SampleType};
                ...
                match channel.sample_type {
                    SampleType::F16 => {
                        ...
                        for (out_byte_0, out_byte_1) in out_byte_tuples {
                            let pixel = u32::from(
                                u16::read_from_native_endian(&mut remaining_bytes_ne)
            ...
            match channel.sample_type {
                SampleType::F16 => {
                    ...
                    for (&in_byte_0, &in_byte_1) in sample_byte_pairs {
                        let difference = u32::from(u16::from_be_bytes([in_byte_0, in_byte_1]));
...
pub fn f32_to_f24(float: f32) -> u32 {
    ...
    let result = if exponent == 0x7f800000 {
        if mantissa != 0 {
            ...
            (exponent >> 8) | mantissa | u32::from(mantissa == 0)
        } else {
            // F is an infinity.
            exponent >> 8
        }
