...
impl<Channels> CroppedChannels<Channels> {
    ...
    pub fn crop_layer(new_bounds: IntegerBounds, layer: Layer<Channels>) -> Layer<Self> {
        Layer {
            channel_data: Self {
                ...
                let channels = self
                    ...
                    .map(|channel: AnyChannel<FlatSamples>| {
                        fn crop_samples<T: Copy>(
                            ...
                            trimmed_lines.flatten().copied().collect() // TODO does this use memcpy?
                        }
...
impl<Cropped, Original> CropResult<Cropped, Original> {
    ...
    pub fn or_none_if_empty(self) -> Option<Cropped> {
        match self {
            Self::Cropped(cropped) => Some(cropped),
            Self::Empty { .. } => None,
        }
    }
...
mod test {
    ...
    #[test]
    fn find_no_bounds() {
        let pixels = [vec![0, 0, 0, 0], vec![0, 0, 0, 0], vec![0, 0, 0, 0]];
